#ifndef SUDOKU_FUNCTIONS_CORRECT_H
#define SUDOKU_FUNCTIONS_CORRECT_H
 ...
 */
struct sudoku_exercice* copy_sudoku_exercice_correct(struct sudoku_exercice* from_data);

/**
 * @brief init_matrice_correct initialise une matrice en allouant dynamiquement de la mémoire, met toute la structure à 0
 * @return un pointer sur la matrice alouée dynamiquement
 */
struct matrice_possibility* init_matrice_correct();
...
/**
 * @brief compute_matrice_correct permet de remplir la matrice en fonction des différentes possibilité du sudoku exercice passé en paramètre
 ...
 */
int compute_matrice_correct(struct matrice_possibility* matrice, struct sudoku_exercice *exerc);

/**
 * @brief compute_possibility_correct coeur de resolution des sudokus. Cette fonction prend en paramètre la matrice des possibilités et recherche la première case
 ...
 *      > une autre valeur : la valeur que l'on a fixé sur cette iteration est valide mais il reste des valeurs à fixer, on doit donc continuer le calcul;
 */
int compute_possibility_correct(struct matrice_possibility* matrice, int* nb_possibility);

/**
 * @brief compute_uplet_correct permet de savoir si la valeur de la matrice à la case line:colonne est valide dans la ligne line et dans la colonne colonne
 ...
 */
int compute_uplet_correct(struct matrice_possibility* matrice, int line, int colonne);

/**
 * @brief contain_char_mpe_correct permet de savoir si une liste de valeurs possibles (une case de la matrice de possibilités ayant plus d'une valeur)
 ...
 */
int contain_char_mpe_correct(char c, struct mpl* e);

/**
 * @brief delete_mpl_correct supprime l'ensemble des elements alloués dynamiquement de la liste passée en paramètre un par un
 ...
 */
void delete_mpl_correct(struct mpl* mpl_to_delete);

/**
 * @brief delete_matrice_correct supprime l'ensemble de la matrice de possibilité, notament toutes les listes de valeurs possibles qui la compose
 ...
 */
void delete_matrice_correct(struct matrice_possibility* matrice);

/**
 * @brief solve_exercice_correct resout completement un exercice sudoku
 ...
 */
struct sudoku_exercice_solve* solve_exercice_correct(struct sudoku_exercice* exerc);
 ...
 * règles du sudoku)
 */
int compute_complexity_correct(struct matrice_possibility* possibility);
 ...
 */
void fill_sudoku_exerc_with_matrice_correct(struct sudoku_exercice* exerc, struct matrice_possibility* possi);

/**
 * @brief delete_exercice_solve supprime une resolution de sudoku
 * @param del l'exercice_solve à supprimer
 */
void delete_exercice_solve(struct sudoku_exercice_solve* del);

#endif // SUDOKU_FUNCTIONS_CORRECT_H
