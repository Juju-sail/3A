...
void init_sudoku_list(struct sudoku_list* list_to_init){
    if(list_to_init->first != NULL){
        ...
        printf("Liste non vide, abandon de l'initialisation\n");
        return;
    }
...
struct sudoku_element* create_end(struct sudoku_list* base_list){
    struct sudoku_element* elem_ptr = malloc(sizeof(struct sudoku_element));
    init_sudoku_element(elem_ptr);
    elem_ptr->prev = get_last_element(base_list);
    if(elem_ptr->prev != NULL){
        elem_ptr->prev->next = elem_ptr;
    }
    else{
        free(elem_ptr);
        creat_front(base_list);
        base_list->nb_element -= 1; //On fait ça car create_front ajoute deja 1 à nb_element (attention avec ça)
    }
    base_list->nb_element += 1;
    return elem_ptr;
}
...
struct sudoku_element *get_last_element(struct sudoku_list *list){
    struct sudoku_element* iter_elem = list->load;
    if(iter_elem == NULL){
        return iter_elem;
    }
...
{
    struct sudoku_element* last_element = get_last_element(list);
...
{
    struct sudoku_element* first_element = get_front_element(list);
...
void init_tab_exercice(struct sudoku_list* list)
{
    ...
    int iter = 0; // position dans le char tab_exercice[]
    int number = 0; // quel sudoku on farique
    char current_tab[82]; // A remplire jusqu'au \n
    memset(current_tab,0,82);
    while(1)
    {
        char current_char = tab_exercice[iter]; // On va chercher dans le tab exos le carractere à la place iter
        ...
        case '\0' :
        {
            return; //car rien à faire, c'est la fin des 200 exos
        }
        ...
        case '\n' :
        {
            ...
            iter++;
            break;
            // affecte la current table à un sudoku exercice (dans sudoku element, dans une sudoku liste)
        }
...
{
    sprintf(dest,"sudoku%04d",num_sudoku); //le 04 c'est pour avoir toujours 4 carracteres (completés par des 0)
}
...
void compute_matrice(struct matrice_possibility* matrice, struct sudoku_exercice *exerc)
{
    for(int iter_column = 0 ; iter_column < 9 ; iter_column ++)
    {
        for(int iter_line = 0 ; iter_line < 9 ; iter_line ++)
        {
            ...
            //2 cas : case vide ou pleine (avec chiffre)
...
void compute_carre(struct matrice_possibility* matrice)
{
    ...
    for(int i=0;i<9;i++){
        for(int j=0;j<9;j++){
            struct mpl* current_matrice = matrice->possibilities[i][j];
            if(current_matrice->nb_elem == 1){
                int modulo_line = i%3;
                int modulo_col = j%3;
                char element_to_verrify = current_matrice->load->element;
                ...
                case 0 :{
                    ...
                    case 0 :{
                        ...
                        tab_mpl[0] = matrice->possibilities[i+1][j+1];
                        tab_mpl[1] = matrice->possibilities[i+1][j+2];
                        tab_mpl[2] = matrice->possibilities[i+2][j+1];
                        tab_mpl[3] = matrice->possibilities[i+2][j+2];

                        for(int iter_mpl = 0; iter_mpl<4; iter_mpl++){
                            if(tab_mpl[iter_mpl]->nb_elem>1){
                                delete_char_mpe(element_to_verrify,tab_mpl[iter_mpl]);
                            }
                        }
                        break;
                    }
                    case 1 :{
                        tab_mpl[0] = matrice->possibilities[i+1][j-1];
                        tab_mpl[1] = matrice->possibilities[i+1][j+1];
                        tab_mpl[2] = matrice->possibilities[i+2][j-1];
                        tab_mpl[3] = matrice->possibilities[i+2][j+1];
                    ...
                    case 2 : {
                        tab_mpl[0] = matrice->possibilities[i+1][j-2];
                        tab_mpl[1] = matrice->possibilities[i+1][j-1];
                        tab_mpl[2] = matrice->possibilities[i+2][j-2];
                        tab_mpl[3] = matrice->possibilities[i+2][j-1];
