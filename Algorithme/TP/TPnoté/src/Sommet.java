...
public class Sommet{
	...
	//Méthode ajout sommet dans une liste (qui du coup contient tous les voisons d'un nb)
	public void addVoisin(Sommet s) {
		voisin.add(s); //fonction toute faite pour ajouté un element dans un vecteur
	}
	
	//méthode return true si s est un voisin
	public boolean isVoisin(Sommet s) {
		return voisin.contains(s); //Dans voisins, est-ce que s est là ?
	}
